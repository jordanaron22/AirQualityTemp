...
# radius <- 20
GetPMDates <- function(pm_data, lon_list_ap, lat_list_ap, radius,rcoord){
  rdates <- c()
  for (i in 1:dim(rcoord)[1]){
    ...
    if (!is.na(working_pm[[1]][[1]])){
      working_pm <- filter(working_pm,Date > as.Date("2019-01-05"))
      rdate <- working_pm$Date[sample(sort(working_pm$armean, 
                                           index.return=TRUE, 
                                           decreasing=TRUE,na.last = T)$ix[1:2],1)]
...
rdates <- GetPMDates(pm_frm_nfrm.df, lon_list_ap, lat_list_ap,20,rcoord) + 2
...
patient_exposure_2019 <- GetExposureData(patient.df,pm_frm_nfrm.df,o3_combined.df,2019,lon_list2_ap,lat_list2_ap,lon_list_o3,lat_list_o3,lon_list_temp,lat_list_temp)
...
```{r}
library(survival)
patient_exposure_2019 <- patient_exposure
...
summary(clogit(event ~ armean_4 + armean_3 + armean_2 + armean_1 + armean_0 +
